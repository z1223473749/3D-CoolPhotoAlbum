...
// 过渡效果数组
    ...
    const transitionEffectsA = [
        "淡入淡出", "缩放", "向上滑动", "向下滑动", "向左滑动", "向右滑动", "模糊", "缩放模糊", "旋转", "水平翻转", "垂直翻转",
        "放大", "缩小", "360度旋转", "X轴倾斜", "Y轴倾斜", "透视", "脉冲", "弹跳", "收缩", "扩展", "闪烁",
        "抖动", "增长", "拉伸", "扭曲", "向左弹跳", "向右弹跳", "X轴抖动", "Y轴抖动", "淡入", "淡出", "摇摆",
        "无限故障", "催眠", "彩虹发光", "破碎效果", "像素化", "倾斜模糊", "波纹效果", "爆炸效果", "漩涡效果", "闪烁效果"
    ];
    let transitionEffects = transitionEffectsA


    // 获取容器元素
    const container = document.getElementById('transitionOptions');

// 动态生成复选框
    transitionEffectsA.forEach(effect => {
        // 创建复选框容器
        ...
        checkbox.name = 'transitionEffect';
        checkbox.value = effect;
        checkbox.checked = true; // 初始数组成员为选中状态
        checkbox.className = 'transition-option';
        // 创建标签
...
// 监听过度效果选择状态变化
    container.addEventListener('change', event => {
        if (event.target.name === 'transitionEffect') {
            const effect = event.target.value;

            // 如果复选框被选中，则添加到数组
            if (event.target.checked) {
                if (!transitionEffects.includes(effect)) {
                    transitionEffects.push(effect);
                }
            }
            // 如果复选框取消选中，则从数组中移除
            else {
                transitionEffects = transitionEffects.filter(item => item !== effect);
            }
        }
    });
    const resetButton = document.getElementById("transitionOptionsB_Reset");
    const cancelButton = document.getElementById("transitionOptionsB_Cancel");
    const checkboxes = document.querySelectorAll(".transition-option");
    // 过度效果全部选中并更新 transitionEffects 数组
    resetButton.addEventListener("click", () => {
        checkboxes.forEach(checkbox => {
            checkbox.checked = true; // 将复选框选中
            const effect = checkbox.getAttribute("data-effect");
            if (!transitionEffects.includes(effect)) {
                transitionEffects.push(effect); // 如果该效果还没有加入数组，则加入
            }
        });
    });

// 过度效果全部取消并清空 transitionEffects 数组
    cancelButton.addEventListener("click", () => {
        checkboxes.forEach(checkbox => {
            checkbox.checked = false; // 取消选中复选框
        });
        // 清空数组
        transitionEffects.length = 0; // 清空 transitionEffects 数组
    });
